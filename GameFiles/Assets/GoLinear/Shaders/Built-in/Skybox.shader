Shader "Linear Lighting/RenderFX/Skybox" {
	Properties {
	...
	SubShader {
		...
		{
			fixed4 tex = LLDecodeTex( tex2D (smp, i.texcoord) );
			fixed4 col;
			col.rgb = tex.rgb + LLDecodeGamma( _Tint.rgb ) - unity_ColorSpaceGrey;
			col.a = tex.a * LLDecodeGamma( _Tint.a );
			return col;
		}
		...
		Pass {
			...
			sampler2D _FrontTex;
			fixed4 frag (v2f i) : COLOR { return LLEncodeGamma( skybox_frag(i,_FrontTex) ); }
		...
		Pass {
			...
			#pragma fragmentoption ARB_precision_hint_fastest
			sampler2D _BackTex;
			fixed4 frag (v2f i) : COLOR { return LLEncodeGamma( skybox_frag(i,_BackTex) ); }
		...
		Pass {
			...
			#pragma fragmentoption ARB_precision_hint_fastest
			sampler2D _LeftTex;
			fixed4 frag (v2f i) : COLOR { return LLEncodeGamma( skybox_frag(i,_LeftTex) ); }
		...
		Pass {
			...
			#pragma fragmentoption ARB_precision_hint_fastest
			sampler2D _RightTex;
			fixed4 frag (v2f i) : COLOR { return LLEncodeGamma( skybox_frag(i,_RightTex) ); }
		...
		Pass {
			...
			#pragma fragmentoption ARB_precision_hint_fastest
			sampler2D _UpTex;
			fixed4 frag (v2f i) : COLOR { return LLEncodeGamma( skybox_frag(i,_UpTex) ); }
		...
		Pass {
			...
			#pragma fragmentoption ARB_precision_hint_fastest
			sampler2D _DownTex;
			fixed4 frag (v2f i) : COLOR { return LLEncodeGamma( skybox_frag(i,_DownTex) ); }
